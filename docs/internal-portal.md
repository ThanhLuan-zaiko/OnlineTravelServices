...
Cổng nội bộ nằm tại `/internal`, được thiết kế cho nhân sự vận hành nội bộ. Hệ thống hiện có hai role staff đăng nhập và một hồ sơ Admin tổng seed-only:
...
- Admin tổng: dùng role `administrative_staff`, nhưng phải có `staff_level = super_admin` và quyền `staff:manage`, `system:manage`.
...
`AdminSuperStaff` được tạo riêng bằng `bun run seed:admin-super-staff` và có quyền truy cập:

- Tất cả chức năng của `AdministrativeStaff`.
- Tất cả chức năng của `OperationsAndStatisticsStaff`.
- Module `/internal/admin/staff/*` để quản lý nhân sự, phân quyền, vai trò, trạng thái và hiệu suất.
- Module `/internal/admin/system/*` để quản lý task hệ thống, backup, restore, maintenance và health.

Ranh giới bảo mật bắt buộc:

- `seed:administrative-staff` không cấp `operations:access`, `staff:manage`, `system:manage`.
- `seed:operations-statistics-staff` không cấp `staff:manage`, `system:manage`.
- API quản lý staff không cho tạo/cập nhật staff có `staff_level = super_admin` hoặc quyền `staff:manage`, `system:manage`; Admin tổng chỉ được tạo qua seed riêng.
...
| `/internal/admin` | Tổng quan Admin tổng |
| `/internal/admin/staff/list` | Danh sách nhân sự nội bộ |
| `/internal/admin/staff/create` | Tạo nhân sự nội bộ thường |
| `/internal/admin/staff/roles` | Xem bộ quyền theo vai trò |
| `/internal/admin/staff/permissions` | Ma trận quyền Admin tổng/Admin/Operations |
| `/internal/admin/staff/activity` | Theo dõi hoạt động nhân sự |
| `/internal/admin/staff/disabled` | Nhân sự inactive/suspended |
| `/internal/admin/customers/violations` | Xử lý khách hàng vi phạm |
| `/internal/admin/customers/segments` | Phân nhóm và hạng khách hàng |
| `/internal/admin/revenue/taxes` | Góc nhìn thuế |
| `/internal/admin/revenue/losses` | Góc nhìn lỗ/hoàn tiền |
| `/internal/admin/operations/*` | Truy cập toàn bộ module Operations dưới namespace Admin tổng |
| `/internal/admin/system/tasks` | Task hệ thống |
| `/internal/admin/system/jobs` | Job đang chạy |
| `/internal/admin/system/backups` | Backup dữ liệu |
| `/internal/admin/system/restore` | Restore dữ liệu |
| `/internal/admin/system/maintenance` | Bảo trì hệ thống |
| `/internal/admin/system/health` | Sức khỏe hệ thống |

## ◈ API Nội Bộ

API dưới `/api/internal/*` yêu cầu session staff. API nghiệp vụ administrative yêu cầu role `administrative_staff`; API `/api/internal/operations/*` yêu cầu permission `operations:access`; API `/api/internal/admin/*` yêu cầu hồ sơ Admin tổng seed-only.
...
| `GET/POST /api/internal/admin/staff` | Danh sách và tạo staff thường, chỉ Admin tổng |
| `GET/PATCH/DELETE /api/internal/admin/staff/[staffId]` | Chi tiết, cập nhật, xóa staff thường, chỉ Admin tổng |
| `GET/POST /api/internal/admin/system/tasks` | Danh sách và tạo task hệ thống, chỉ Admin tổng |
| `PATCH /api/internal/admin/system/tasks/[taskId]` | Cập nhật trạng thái task hệ thống, chỉ Admin tổng |
| `GET/POST /api/internal/admin/system/backups` | Danh sách và tạo backup, chỉ Admin tổng |
| `POST /api/internal/admin/system/restore` | Restore backup, chỉ Admin tổng |
| `POST /api/internal/admin/system/maintenance` | Chạy maintenance hệ thống, chỉ Admin tổng |
...
Script seed ghi vào cùng các bảng `users_by_*` và `staff_*`, với role `administrative_staff`, `staff_level = super_admin`.

Quyền mặc định là union của AdministrativeStaff + OperationsAndStatisticsStaff, cộng thêm quyền cấp hệ thống:
...
- Khi deploy, đưa biến `ADMIN_SUPER_STAFF_*`, `ADMINISTRATIVE_STAFF_*` và `OPERATIONS_STATISTICS_STAFF_*` vào secret manager hoặc chỉ set tạm trong lúc seed.
- Không cấp `staff:manage` hoặc `system:manage` cho staff thường qua script, UI, API hoặc migration dữ liệu.
