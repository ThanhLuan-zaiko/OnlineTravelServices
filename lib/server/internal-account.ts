...
import { getCurrentAdministrativeStaff, getCurrentInternalStaff } from "@/lib/server/auth";
...
async function findStaffByUserId(role: string, userId: string) {
...
function buildProfile(userId: string, user: NonNullable<Awaited<ReturnType<typeof findUserById>>>, details: StaffByIdRow): InternalAccountProfile {
  return {
    ...
    role: details.role as InternalAccountProfile["role"],
...
export async function getCurrentAdministrativeStaffProfile(cookieValue: string | undefined) {
  ...
  const staff = await findStaffByUserId(currentUser.role, currentUser.userId);
...
export async function getCurrentInternalStaffProfile(cookieValue: string | undefined) {
  const currentUser = await getCurrentInternalStaff(cookieValue);

  if (!currentUser) {
    throw new AuthError("Vui lòng đăng nhập bằng tài khoản nhân sự nội bộ.", 401);
  }
...
export async function updateCurrentInternalStaffProfile(
  cookieValue: string | undefined,
  input: InternalAccountProfileRequest,
) {
  const currentUser = await getCurrentInternalStaff(cookieValue);
