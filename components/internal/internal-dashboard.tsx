    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {stats.map((stat) => {
          ...
          return (
            <Link
              className="rounded-xl border border-slate-200 bg-white p-4 transition hover:border-slate-300 hover:shadow-sm dark:border-neutral-900 dark:bg-black dark:hover:border-neutral-700"
              href={stat.href}
              key={stat.label}
            >
              <div className="flex items-center justify-between gap-3">
                ...
                <FiArrowRight className="text-slate-400" size={17} />
              </div>
              <p className="mt-4 text-2xl font-semibold text-slate-950 dark:text-neutral-50">{stat.value}</p>
              <p className="mt-1 text-sm font-medium text-slate-500 dark:text-neutral-400">{stat.label}</p>
            </Link>
          );
        })}
      </div>
