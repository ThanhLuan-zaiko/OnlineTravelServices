...
import { getInternalVehicleCatalogPage } from "@/lib/client/api-client";

type UseVehicleCursorPageInput = {
  fixedStatus?: string;
...
export function useVehicleCursorPage({
  fixedStatus,
  initialStatus,
  missingNextMessage,
  missingPageMessage,
  showToast,
}: UseVehicleCursorPageInput) {
  ...
  const jumpToPage = async (targetPage: number, nextCursor: string | null) => {
    ...
    try {
      ...
      for (let page = currentPage + 1; page < targetPage; page += 1) {
        const response = await getInternalVehicleCatalogPage({
