    ...
    <div className="space-y-5">
      ...
      {activeTab === "manage" ? (
        <div className="grid gap-5 xl:grid-cols-[0.95fr_1.05fr]">
          ...
          <MediaPanel disabled={!editing || saveMutation.isPending} emptyLabel={editing ? "Chưa có ảnh trong gallery." : "Lưu dịch vụ trước khi upload ảnh."} fileSelectionDisabled={saveMutation.isPending} media={media as MediaItem[]} mediaPending={mediaQuery.isLoading} onDeleteMedia={(item) => setDangerAction({ kind: "delete-media", media: item as InternalServiceMedia })} onSelectFiles={setSelectedFiles} onSetCover={(item) => setCoverMutation.mutate(item as InternalServiceMedia)} onUpload={() => uploadMutation.mutate()} selectedFiles={selectedFiles} targetLabel={editing ? `Ảnh của ${editing.name}` : "Ảnh dịch vụ"} uploadPending={uploadMutation.isPending} />
        </div>
      ) : null}
